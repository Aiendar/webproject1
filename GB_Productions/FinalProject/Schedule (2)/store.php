<html>
	<?php 
		...
		if($db->affected_rows == 0 && $event != null && $date != null){
			...
			$db->query($query)
			or die($db->error);
			header( 'Location: http://localhost/xampp/FinalProject/EventScheduler/' ) ;
		}
		else{
			...
			$string .=';if (r == true) {console.log("something"); }';
			 $string .= 'else {console.log("error");window.location.replace("http://localhost/xampp/FinalProject/EventScheduler/index.html");}</script>';
			 echo $string; 
		}
